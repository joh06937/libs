...
```cmake
set(KCONFIG_FILE "${CMAKE_CURRENT_LIST_DIR}/path/to/Kconfig")
...
```bash
$ cmake -B build -G Ninja -D KCONFIG_FILE=./path/to/Kconfig
```
...
CMLib's Kconfig process allows users to manually specify root Kconfig
configuration option selection files using a `PROJECT_FILES` variable, with
paths separated by a comma. The variable must be set prior to the package's
inclusion:

```cmake
set(PROJECT_FILES "${CMAKE_CURRENT_LIST_DIR}/path/to/prj.conf,"${CMAKE_CURRENT_LIST_DIR}/path/to/prj-extra.conf")
...
```bash
$ cmake -B build -G Ninja -D PROJECT_FILES=./path/to/prj.conf,./path/to/prj-extra.conf
```
...
```cmake
set(IGNORE_USER_FILE TRUE)
```

Alternatively, the variable can be specified as part of configuring CMake:

```bash
$ cmake -B build -G Ninja -D IGNORE_USER_FILE=1
```

If the file exists and isn't restricted, it will be automatically appended to
the _end_ of the list of files in `PROJECT_FILES`. The appending to the end is
significant for Kconfig, as that means its options will take precedence over any
previous option selections.
...
After running the Kconfig process and determining the final option selections,
CMLib will pull the configuration selections into the CMake scope using cache
variables. The names of the variables will match the _output_ names of the
Kconfig options. Note that these names are a convention of Kconfig: an option
named `FOO` will have a usable name of `CONFIG_FOO`.

For example, if there is a Kconfig option:
...
and the option is selected by the project:

```
CONFIG_FOO=y
```

the option can be used by a CMake file in the project:

```cmake
if(CONFIG_FOO)
    target_sources(${APP_NAME} PRIVATE foo.cpp)
endif()
```

#### C/C++ Source Code

Source code can use the selected Kconfig options by including the
Kconfig-generated `autoconf.h` output file, which is automatically put into the
include path list by CMLib:
